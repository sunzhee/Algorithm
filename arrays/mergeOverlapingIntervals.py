...
intervals, merges any overlapping intervals, and returns the new
intervals in no particular order.
Each interval interval is an array of two integers, with
interval[0] as the start of the interval and interval[1]
as the end of the interval.
Note that back-to-back intervals aren't considered to be
overlapping. For example, [1, 5] and [6, 7] aren't
overlapping; however, [1, 6] and [6, 7] are indeed
overlapping.
...
def mergeOverlappingIntervals(intervals):
	outputResult = []
	sortedInputs = sorted(intervals,key=lambda x: x[0])
	
	currentInterval = sortedInputs[0]
	outputResult.append(currentInterval)
	for nextInterval in sortedInputs:
		nextIntervalStart,nextIntervalEnd = nextInterval
		currentIntervalStart,currentIntervalEnd = currentInterval
		if currentIntervalEnd >= nextIntervalStart:
			currentInterval[1] = max(currentIntervalEnd,nextIntervalEnd)
		else:
			outputResult.append(nextInterval)
			currentInterval = nextInterval
	return outputResult

print("input:",intervals)
print("output:",mergeOverlappingIntervals(intervals))